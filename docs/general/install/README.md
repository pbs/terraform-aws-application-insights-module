...
module "application-insights" {
    source = "github.com/pbs/terraform-application-insights-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/application-insights'
gh -R 'pbs/terraform-application-insights-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/application-insights'
rm -f release.tar.gz
```
...
```bash
git remote add -f application-insights git@github.com:pbs/terraform-application-insights-module.git
git subtree add --prefix terraform/modules/application-insights application-insights main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/application-insights application-insights $REF --squash
```
